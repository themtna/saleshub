...
export default function LoginPage({ onLogin }) {
  ...
  const handleLogin = async () => {
    ...
    try { await onLogin({ email, password }) } catch (err) { setError(err.message || 'อีเมลหรือรหัสผ่านไม่ถูกต้อง') }
    setLoading(false)
  }
  ...
  return (
    <div style={{ fontFamily: T.font, minHeight: '100vh', background: `linear-gradient(170deg, #FAFAF7 0%, #F5F0E0 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, color: T.text }}>
      <div style={{ ...glass, padding: '40px 30px', width: '100%', maxWidth: 400, boxShadow: '0 8px 40px rgba(184,134,11,0.08)', border: `1px solid ${T.borderGold}` }}>
        ...
        <FI label="อีเมล" type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" onKeyDown={e => e.key === 'Enter' && handleLogin()} />
        <FI label="รหัสผ่าน" type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" onKeyDown={e => e.key === 'Enter' && handleLogin()} />
